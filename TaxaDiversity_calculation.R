...
all <- read.csv("All_000_noDups.csv", h = T, sep = ",", stringsAsFactors = FALSE, check.names=FALSE)
head(all)

#individual-based rareification of species richness
...
for(i in unique(all$site_id)){
  ...
  sub.m_r <- dcast(sub, code ~ taxon_id, sum, value.var = "ro.ab")      # matrix form for rareifaction with rounded richness
