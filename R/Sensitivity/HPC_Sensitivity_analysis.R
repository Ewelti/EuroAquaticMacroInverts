...
fit1 <- brm(estimate|se(sd) ~ -1 + season + (1|study_id) + (1|country),
...
fit1 <- brm(estimate|se(sd) ~ -1 + TaxonomicRes + (1|study_id) + (1|country),
            data = response_stan, iter=4000, chains = 4,
            prior = prior1)
