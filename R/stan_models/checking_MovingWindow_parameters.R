...
unique(ma_ests$Response)
fred_est <- ma_ests[ma_ests$Response == "FRed",]
fric_est <- ma_ests[ma_ests$Response == "FRic",]
...
fric_est$MeanYear <- as.numeric(fric_est$StartYear) + 4.5
fric_est_mw <- fric_est[fric_est$MeanYear > 1994,]
fric_est_mw <- fric_est_mw[fric_est_mw$MeanYear < 2016,]

fred_est$MeanYear <- as.numeric(fred_est$StartYear) + 4.5
fred_est_mw <- fred_est[fred_est$MeanYear > 1994,]
fred_est_mw <- fred_est_mw[fred_est_mw$MeanYear < 2016,]
...
max(fric_est_mw$Est.Error)
plot(fric_est_mw$Est.Error ~fric_est_mw$MeanYear, ylim=c(0,0.015),xlim= c(1994,2016), pch=19,
ylab="Estimated error of functional richness trend", xlab="Mean year of moving window")
points (fric_est_mw$Est.Error ~fric_est_mw$MeanYear, type="l")

max(fred_est_mw$Est.Error)
plot(fred_est_mw$Est.Error ~fred_est_mw$MeanYear, ylim=c(0,0.002),xlim= c(1994,2016), pch=19,
ylab="Estimated error of functional redundancy trend", xlab="Mean year of moving window")
points (fred_est_mw$Est.Error ~fred_est_mw$MeanYear, type="l")
...
fric_pos <- fric[fric$estimate > 0,]
fric_neg <- fric[fric$estimate < 0,]
fred_pos <- fred[fred$estimate > 0,]
fred_neg <- fred[fred$estimate < 0,]
...
sitecount_sr_neg
DT <- data.table(fric_pos)
sitecount_fric_pos <- DT[, .(site_num_fric_pos = length(unique(site_id))), by = StartYear]
DT <- data.table(fric_neg)
sitecount_fric_neg <- DT[, .(site_num_fric_neg = length(unique(site_id))), by = StartYear]
sitecount_fric_neg
DT <- data.table(fred_pos)
sitecount_fred_pos <- DT[, .(site_num_fred_pos = length(unique(site_id))), by = StartYear]
DT <- data.table(fred_neg)
sitecount_fred_neg <- DT[, .(site_num_fred_neg = length(unique(site_id))), by = StartYear]
sitecount_fred_neg

MoAv1 <- merge(sitecount_ab_pos,sitecount_ab_neg,by="StartYear", all=T)
MoAv2 <- merge(MoAv1,sitecount_sr_pos,by="StartYear", all=T)
MoAv3 <- merge(MoAv2,sitecount_sr_neg,by="StartYear", all=T)
MoAv4 <- merge(MoAv3,sitecount_fric_pos,by="StartYear", all=T)
MoAv5 <- merge(MoAv4,sitecount_fric_neg,by="StartYear", all=T)
MoAv6 <- merge(MoAv5,sitecount_fred_pos,by="StartYear", all=T)
count_sign <- merge(MoAv6,sitecount_fred_neg,by="StartYear", all=T)
head(count_sign)

count_sign$ab_posnegR <- count_sign$site_num_Abund_pos/(count_sign$site_num_Abund_neg+count_sign$site_num_Abund_pos)
count_sign$sr_posnegR <- count_sign$site_num_SRich_pos/(count_sign$site_num_SRich_neg+count_sign$site_num_SRich_pos)
count_sign$fric_posnegR <- count_sign$site_num_fric_pos/(count_sign$site_num_fric_neg+count_sign$site_num_fric_pos)
count_sign$fred_posnegR <- count_sign$site_num_fred_pos/(count_sign$site_num_fred_neg+count_sign$site_num_fred_pos)
count_sign$MeanYear <- as.numeric(count_sign$StartYear) + 4.5
max(na.omit(count_sign$fric_posnegR))
...
par(mfrow=c(2,2),mar=c(3,5,0.2,0.2))
plot(0,0, ylim=c(0.4,0.67),xlim= c(1994,2016), ylab="", xlab="",las=1)
...
#mtext(side=4, "Number of sites", line=3, las=3)
...
#axis(4, at=c(0.797940009, 1.098970004, 1.275061263, 1.4, 1.496910013, 1.576091259),cex.axis=1,labels=marks,las=1,col="cyan4")
#mtext(side=4, "Number of sites", line=3)

plot(0,0, ylim=c(0.4,0.67),xlim= c(1994,2016), ylab="", xlab="", las=1)
polygon(x = c(1, 1, 2040, 2040), y = c(-100, 0.5, 0.5, -100), col ="grey80", border = NA)
#points (transnum ~ count_sign_mw$MeanYear, type="l", col="cyan4")
#points (transnum ~ count_sign_mw$MeanYear, pch=19, col="cyan4")
points (count_sign_mw$fric_posnegR ~count_sign_mw$MeanYear, pch=19)
points (count_sign_mw$fric_posnegR ~count_sign_mw$MeanYear, type="l")
box(lwd=2)
title(ylab="Proportion of sites with positive trends", line=2.9)
title(xlab="Mean year of moving window", line=2)
legend("bottomright", bty="n", legend="c, Func. richness",cex=1.5)
#legend("topleft", bty="n", legend=c("trend ratio", "site count"), pch=19, col=c(1,"cyan4"))
#axis(4, at=c(0.797940009, 1.098970004, 1.275061263, 1.4, 1.496910013, 1.576091259),cex.axis=1,labels=marks,las=1,col="cyan4")
#mtext(side=4, "Number of sites", line=3)
...
#points (transnum ~ count_sign_mw$MeanYear, type="l", col="cyan4")
#points (transnum ~ count_sign_mw$MeanYear, pch=19, col="cyan4")
points (count_sign_mw$fred_posnegR ~count_sign_mw$MeanYear, pch=19)
points (count_sign_mw$fred_posnegR ~count_sign_mw$MeanYear, type="l")
box(lwd=2)
title(ylab="Proportion of sites with positive trends", line=2.9)
title(xlab="Mean year of moving window", line=2)
legend("topright", bty="n", legend="d, Func. redundancy",cex=1.5)
#legend("topleft", bty="n", legend=c("trend ratio", "site count"), pch=19, col=c(1,"cyan4"))
#axis(4, at=c(0.797940009, 1.098970004, 1.275061263, 1.4, 1.496910013, 1.576091259),cex.axis=1,labels=marks,las=1,col="cyan4")
#mtext(side=4, "Number of sites", line=3)
