...
# attach data
drivEst <- read.csv("outputs/outputs_sensitivity/SppLevel_outputs/Driver_sppLevel_metaanaly_Ests.csv")
head(drivEst)
...
##save plot
tiff(filename = "plots/Fig4_drivers/Drivers_main_SppLevel_horseshoe.tiff", width = 7.8, height = 5, units = 'in', res = 600, compression = 'lzw')
...
sp <- d1[which(d1$TaxonomicRes == "species"),]
ge <- d1[which(d1$TaxonomicRes == "genus"),]
fa <- d1[which(d1$TaxonomicRes == "family"),]

boxplot(d1$ppt_mm_12moPrior ~ d1$TaxonomicRes, xlab="",ylab="Mean annual precipitation (mm)")
boxplot(d1$ppt_Est ~ d1$TaxonomicRes, xlab="",ylab="Precipitation trend")
boxplot(d1$dam_impact_score_lessthan100km ~ d1$TaxonomicRes, xlab="",ylab="Dam Impact Score")
...
mean(sp$dam_impact_score_lessthan100km)
se(sp$dam_impact_score_lessthan100km)

mean(d1$dam_impact_score_lessthan100km)
se(d1$dam_impact_score_lessthan100km)
