...
# attach data
MA <- read.csv("outputs/HighThresholdMovingAve_YrEsts.csv")
...
tiff(filename = "plots/Fig3_movingWindow/HighThreshold_MovingAverages.tiff", width = 8, height = 6.5, units = 'in', res = 600, compression = 'lzw')
par(mfrow=c(2,2),mar=c(2,4,0.2,0.2))

#plot for spp richness
SR <- subset(MA, Response == "higherthresholdspp_richness")
...
plot(SRs$Estimate~SRs$meanYr,ylab="", cex=1.5, xlab="", type="n", las=1, ylim=c(-1.8,4.1), xlim=c(2004.5,2015.5))#ylim=c(-0.8,2.55))
...
#plot for abundance
SR <- subset(MA, Response == "higherthresholdabundance")
...
plot(SRs$Estimate~SRs$meanYr,ylab="", cex=1.5, xlab="", type="n", las=1, ylim=c(-5.5,5.3), xlim=c(2004.5,2015.5)) #ylim=c(-0.57,0.4))
...
#plot for FRic
SR <- subset(MA, Response == "higherthresholdFRic")
st <-(SR[1:(nrow(SR)),3:12])*100
sr <- cbind(SR$StartYear, SR$site_num, SR$meanYr, st)
names(sr)[names(sr) == 'SR$StartYear'] <- 'StartYear'
names(sr)[names(sr) == 'SR$site_num'] <- 'site_num'
...
plot(SRs$Estimate~SRs$meanYr,ylab="", cex=1.5, xlab="", type="n", las=1, ylim=c(-8,6), xlim=c(2004.5,2015.5)) #ylim=c(-0.57,0.4))
...
#plot for FRed
SR <- subset(MA, Response == "higherthresholdFRed")
...
plot(SRs$Estimate~SRs$meanYr,ylab="", cex=1.5, xlab="", type="n", las=1, ylim=c(-2.2,3.5), xlim=c(2004.5,2015.5)) #ylim=c(-0.57,0.4))
