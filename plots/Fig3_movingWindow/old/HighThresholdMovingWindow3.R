...
# attach data
MA <- read.csv("outputs/HighThresholdMovingAve3_YrEsts.csv")
MA$meanYr <- (MA$StartYear+ 4.5)
head(MA)
MA = subset(MA, select = -c(X) )

tiff(filename = "plots/Fig3_movingWindow/HTMW3.tiff", width = 8, height = 6.5, units = 'in', res = 600, compression = 'lzw')
...
SRs <- sr[ which(sr$StartYear >1989 & sr$StartYear <2015), ] # all data

plot(SRs$Estimate~SRs$meanYr,ylab="", cex=1.5, xlab="", type="n", las=1, ylim=c(min(sr$Q2.5),max(sr$Q97.5)), xlim=c(1994.5,2018.5))#
