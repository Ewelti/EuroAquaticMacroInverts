...
tiff(filename = "plots/descriptive_plots/response_maps/FuncRichRedund_map.tiff", width = 11, height = 5, units = 'in', res = 600, compression = 'lzw')
...
#evenness map
sr_sites <- resp[!is.na(resp$FRic),]
sr_sites <- sr_sites[order(sr_sites$FRic),]
head(sr_sites)

##convert slope to percent annual change
sr_sites$trend_perc <- (sr_sites$FRic)*100

#break positive and neg values
sr_neg <- sr_sites$trend_perc[(1:length(sr_sites$trend_perc))[sr_sites$FRic <= 0]]
sr_pos <- sr_sites$trend_perc[(1:length(sr_sites$trend_perc))[sr_sites$FRic > 0]]
...
x <- rep(-8,co_leng)
x2 <- rep(-6,co_leng)
legend(-9.15, 72.4,title="",legend=lg2,col =c('midnightblue','white','white','white','white','white','red4'),text.col=c("white"),lty=1,lwd=1,bty="n",cex=0.81)
segments(x, y, x2, y, col= unique(sr_sites$sr_col),lwd=3)
legend(-12.5,74,legend=expression(paste("a) Functional richness (%", y^-1, ")", sep = "")),text.col=c("white"),bty='n',cex=1.2)

############################ Functional Redundancy #######################################

sr_sites <- resp[!is.na(resp$FRed),]
sr_sites <- sr_sites[order(sr_sites$FRed),]
head(sr_sites)

##convert slope to percent annual change
ave_trend <- 0.291145846
sr_sites$trend_perc <- (sr_sites$FRed/ave_trend)*100

#break positive and neg values
sr_neg <- sr_sites$trend_perc[(1:length(sr_sites$trend_perc))[sr_sites$FRed <= 0]]
sr_pos <- sr_sites$trend_perc[(1:length(sr_sites$trend_perc))[sr_sites$FRed > 0]]
...
legend(-9.15, 72.4,title="",legend=lg2,col =c('midnightblue','white','white','white','white','white','red4'),text.col=c("white"),lty=1,lwd=1,bty="n",cex=0.81)
segments(x, y, x2, y, col= unique(sr_sites$sr_col),lwd=3)
legend(-12.5,74,legend=expression(paste("b) Functional redundancy (%", y^-1, ")", sep = "")),text.col=c("white"),bty='n',cex=1.2)
